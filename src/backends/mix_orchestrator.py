...
class MixAIOrchestrator(QThread):
    ...
    def set_bible_context(self, bible):
        """v8.0.0: BIBLEコンテキストを設定（v12.6.0: feature_flag制御）"""
        from ..utils.feature_flags import is_bible_enabled
        if is_bible_enabled():
            self._bible_context = bible
        else:
            self._bible_context = None
            if bible:
                logger.info("[MixOrchestrator] BIBLE injection skipped (bible.enabled=false in app_settings.json)")
...
- 不要なカテゴリはskip: trueに設定"""
    ...
    def _execute_phase3(self, phase1_answer: str, phase2_results: list[SequentialResult]) -> dict:
        ...
        if provider:
            if "anthropic" in provider:
                ...
                return self._execute_phase3_codex(phase1_answer, phase2_results, engine)
            elif "google" in provider:
    ...
    def _execute_phase3_codex(self, phase1_answer: str, phase2_results: list[SequentialResult], model_id: str = "") -> dict:
        """Phase 3: GPT-5.3-Codex CLI版（v9.9.0, v12.6.0: model_id対応）"""
        from .codex_cli_backend import run_codex_cli
        from ..utils.model_catalog import normalize_model_id
        clean_model_id = normalize_model_id(model_id, "openai") if model_id else ""
        ...
        raw_output = run_codex_cli(full_prompt, model_id=clean_model_id, effort=effort, run_cwd=run_cwd, timeout=timeout)
        return self._parse_phase3_output(raw_output)
...
- ユーザーへの回答は自然な文章で提示してください。ローカルLLMの存在に言及する必要はありません"""
    ...
    def _execute_phase35(self, phase3_output: dict, model_key: str) -> dict:
        ...
        # レビュープロンプト構築（Phase 1/3 と同じ言語切り替えパターン）
        from ..utils.i18n import get_language
        _lang = get_language()
        _lang_line = "Respond in English." if _lang == "en" else "日本語で回答してください。"
        _issue_example = "issue 1, issue 2" if _lang == "en" else "問題点1, 問題点2"
        _fix_example = "Fix instructions (only for minor_fix)" if _lang == "en" else "軽微な修正指示（minor_fixの場合のみ）"
...
{{
  ...
  "issues": ["{_issue_example}"],
  "fix_instructions": "{_fix_example}"
}}
```
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                try:
                    ...
                    # v12.7.1: JSON内の is_error フラグも確認
                    if output_data.get("is_error"):
                        err_detail = output_data.get("result", "")[:300]
                        logger.warning(f"[Claude CLI] API returned is_error=true: {err_detail}")
                        self.monitor_event.emit("error", effective_model, "API error")
                        raise RuntimeError(f"Claude CLI API エラー: {err_detail or '詳細なし'}")
                    return output_data.get("result", stdout)
                except json.JSONDecodeError:
                    return stdout.strip()
            else:
                # v12.7.1: 詳細ログ（stderr / stdout 両方記録）
                logger.error(
                    f"[Claude CLI] exit code {result.returncode} | "
                    f"stderr={stderr[:300]!r} | stdout={stdout[:300]!r} | "
                    f"model={effective_model} | cwd={run_cwd}"
                )
                # v12.7.1: stdout に JSON エラー情報が含まれる場合も確認
                error_detail = stderr.strip()
                if not error_detail and stdout.strip():
                    try:
                        err_json = json.loads(stdout)
                        if err_json.get("is_error"):
                            error_detail = err_json.get("result", "")[:300]
                    except (json.JSONDecodeError, AttributeError):
                        error_detail = stdout.strip()[:300]

                # v10.1.0: モニターイベント - エラー
                self.monitor_event.emit("error", effective_model, f"exit code {result.returncode}")
                raise RuntimeError(
                    f"Claude CLI の実行に失敗しました (code {result.returncode})"
