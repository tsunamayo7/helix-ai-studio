...
| **soloAI チャット** | Cloud AI（Claude・GPT・Gemini）と Ollama を統合した単一チャット。旧 cloudAI + localAI を統合し、モデルを自由に切り替えて会話できる |
| **CloudAI 設定 / Ollama 設定** | クラウドモデル管理・認証方式・MCP、Ollama 接続・モデル管理を専用タブで設定 |
| **Virtual Desktop** | 隔離された仮想デスクトップ内で AI がコードを書いて実行。Windows Sandbox 標準 / Docker optional |
...
### 好きなモデルを選んでチャット（soloAI）

旧 cloudAI と localAI を統合した soloAI タブ。Claude・GPT・Gemini・Ollama をドロップダウンで切り替え、モデルを変えても会話の流れはそのまま。

![soloAI Demo](docs/demo/gif/demo_cloudai_models.gif)

### ローカル LLM でコード生成（soloAI / Ollama）

Ollama の qwen3.5（122B）が Python コードを生成。soloAI タブから Ollama モデルを選ぶだけ。インターネット不要、API 費用ゼロ。

![soloAI localAI Demo](docs/demo/gif/demo_localai_chat.gif)

### AI が作ったアプリをその場で確認（Virtual Desktop）

隔離された仮想デスクトップ内でアプリが動く（Windows Sandbox 標準 / Docker optional）。あなたの PC 環境は一切汚れません。
...
### mixAI パイプライン
![mixAI パイプライン](docs/demo/best/README/readme_01_mixai_main.png)

### soloAI チャット（Cloud AI）
![soloAI Cloud](docs/demo/best/README/readme_02_soloai_unified_cloud.png)

### soloAI チャット（Ollama ローカル）
![soloAI Ollama](docs/demo/best/README/readme_03_soloai_unified_ollama.png)

### CloudAI設定
![CloudAI設定](docs/demo/best/README/readme_04_cloud_settings.png)

### Ollama設定
![Ollama設定](docs/demo/best/README/readme_05_ollama_settings.png)

### 会話履歴（History）
![History](docs/demo/best/README/readme_06_history.png)

### RAG ナレッジベース
![RAG構築](docs/demo/best/README/readme_07_rag.png)

### Virtual Desktop
![Virtual Desktop](docs/demo/best/README/readme_08_virtual_desktop.png)

### 一般設定
![一般設定](docs/demo/best/README/readme_09_general_settings.png)
...
| バージョン | 主な変更 |
|----------|---------|
| **v12.8.0** | SoloAI Unification — 旧 cloudAI + localAI を soloAI に統合、8 タブ構成（CloudAI 設定・Ollama 設定を分離） |
| **v12.7.0** | Windows Sandbox デフォルト化、バックエンド抽象化レイヤー |
| **v12.5.0** | CrewAI 統合、MCP 全タブ対応、3+1 Phase パイプライン |
| **v12.0.0** | Virtual Desktop（sandbox）、タブ構成刷新 |
