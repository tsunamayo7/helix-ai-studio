# BIBLE — Helix AI Studio v8.3.2 "Living Memory"

**最終更新**: 2026-02-10
**バージョン**: 8.3.2 "Living Memory" (Patch 2)
**前バージョン**: 8.3.1 "Living Memory" (Patch)
...
| `episode_summaries` | id, level, period_start, period_end, summary, summary_embedding, episode_ids, created_at, **status** | RAPTOR多段要約（session/weekly/**version**） |
...
| level | TEXT | 'session', 'weekly', または 'version'（v8.3.1追加） |
...
│   └── BIBLE_Helix AI Studio_8.3.2.md   # ★ 本ファイル
├── config/                       # 設定ファイル
│   ├── app_settings.json         # ★ v8.3.2: version="8.3.2", claude.default_model="claude-opus-4-6"
...
│   │   ├── mix_orchestrator.py   # 3Phaseオーケストレーター ★ v8.3.1: threading.Thread非同期化
...
│   │   └── risk_gate.py          # リスクゲート
│   ├── tabs/
│   │   ├── claude_tab.py         # soloAIタブ ★ v8.3.1: RaptorWorker(QThread) / v8.3.2: Row 2新規セッション削除
...
│   ├── utils/
│   │   ├── constants.py          # 定数定義 ★ v8.3.2: APP_VERSION="8.3.2"
...
**★ v8.3.1**: `claude.default_model` を `"claude-opus-4-6"` に修正（v8.3.0では `"claude-opus-4-5-20250929"` になっていた不整合を修正）。起動時に `HelixAIStudio.py` が自動修正ロジックを実行し、constants.pyのDEFAULT_CLAUDE_MODEL_IDとの整合性を保証。
**★ v8.3.2**: version を `"8.3.2"` に更新。
...
*この BIBLE は Claude Opus 4.6 により、v8.3.1 BIBLE および v8.3.2 パッチ修正（claude_tab.py, bible_panel.py, constants.py, app_settings.json）+ 検証レポート（helix_v831_verification_and_v832_plan.md）に基づいて生成されました。*
*2026-02-10*
