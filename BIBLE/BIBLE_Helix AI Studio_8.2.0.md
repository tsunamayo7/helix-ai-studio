# BIBLE — Helix AI Studio v8.2.0 "RAG Shell"

**最終更新**: 2026-02-10
**バージョン**: 8.2.0 "RAG Shell"
**前バージョン**: 8.1.0 "Adaptive Memory"
...
### 3.9 helix_coreモジュール整理（v8.2.0）

以下の5モジュールはv8.2.0でDEPRECATED指定。実行フローから未参照であることを確認済み。
memory_manager.pyが4層メモリの検索・保存を全て担当するため、これらは次期バージョンで削除予定。

| モジュール | ファイル | 状態 |
|-----------|---------|------|
| memory_store | `src/helix_core/memory_store.py` | DEPRECATED v8.2.0 |
| vector_store | `src/helix_core/vector_store.py` | DEPRECATED v8.2.0 |
| light_rag | `src/helix_core/light_rag.py` | DEPRECATED v8.2.0 |
| rag_pipeline | `src/helix_core/rag_pipeline.py` | DEPRECATED v8.2.0 |
| hybrid_search_engine | `src/helix_core/hybrid_search_engine.py` | DEPRECATED v8.2.0 |
...
│   └── BIBLE_Helix AI Studio_8.2.0.md   # ★ 本ファイル
├── config/                       # 設定ファイル
...
│   │   ├── mix_orchestrator.py   # 3Phaseオーケストレーター ★ v8.2.0: Phase 2 RAG注入追加
...
│   │   ├── hybrid_search_engine.py # ハイブリッド検索  # DEPRECATED v8.2.0
│   │   ├── light_rag.py          # Light RAG  # DEPRECATED v8.2.0
│   │   ├── memory_store.py       # 記憶ストア  # DEPRECATED v8.2.0
│   │   ├── mother_ai.py          # Mother AI
│   │   ├── perception.py         # 知覚モジュール
│   │   ├── rag_pipeline.py       # RAGパイプライン  # DEPRECATED v8.2.0
│   │   ├── vector_store.py       # ベクトルストア  # DEPRECATED v8.2.0
...
│   ├── utils/
│   │   ├── constants.py          # 定数定義 ★ v8.2.0: APP_VERSION="8.2.0"
...
v8.2.0: 不整合修正（CLAUDE_MODELSのIDと統一）
...
| バージョン | 構想 |
|-----------|------|
| v8.3.0 | ベンチマーク機能（Phase 1のみ vs 3Phase統合の品質比較） |
...
*この BIBLE は Claude Opus 4 により、v8.1.0 BIBLE、v8.2.0実装コード、指示書（修正/helix_v820_phase2_rag_shell_upgrade.md）に基づいて生成されました。*
